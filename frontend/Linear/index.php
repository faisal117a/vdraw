...
<head>
    ...
    <title>Data Structures Visualizer – List, Stack, Queue</title>
    <meta name="description" content="Learn core data structures like list, stack, queue, and tuple using visual simulations.">
    <meta name="keywords" content="data structure visualization, stack animation, queue visualization, python list">
...
<body class="h-screen flex overflow-hidden text-slate-100">
        ...
        <div class="p-6 border-b border-slate-800 flex justify-between items-center">
            <div>
                <h1
                    ...
                    <i class="fa-solid fa-layer-group mr-2 text-brand-500"></i>Data Structures
                </h1>
                <p class="text-slate-500 text-xs mt-1 uppercase tracking-wider">Visualize Data Structures</p>
            </div>
            <!-- Close Button (Mobile) -->
    ...
    <main class="flex-1 flex flex-col relative overflow-hidden transition-all duration-300">
        ...
        <!-- Main Content Area - Layout Controller -->
        <div class="h-[calc(100vh-4rem)] relative overflow-hidden flex flex-col">

            <!-- VDraw Dashboard Wrapper -->
            <div id="vdraw-dashboard" class="transition-opacity duration-300 h-full overflow-y-auto custom-scrollbar">
                <!-- Stats overview grid -->
                ...
                <div id="empty-state" class="flex flex-col items-center justify-center h-[60vh] text-center">
                    ...
                    <h3 class="text-xl font-semibold text-slate-300 mb-2">Ready to Analyze</h3>
                    <p class="text-slate-500 max-w-md">Enter data manually or upload a file on the sidebar to generate
                        descriptive statistics and visualizations.</p>
                </div>
            ...
            <div id="pdraw-dashboard" class="hidden transition-opacity duration-300 flex-1 min-h-0 flex flex-col">
                <!-- PDraw Content Placeholder -->
                <div class="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-4 p-4 min-h-0 overflow-y-auto lg:overflow-hidden">
                    ...
                    <div class="lg:col-span-9 flex flex-col h-full">
                        ...
                        <div id="pdraw-empty-state" class="flex-1 flex flex-col items-center justify-center text-center">
                            ...
                            <h3 class="text-xl font-semibold text-slate-300 mb-2">Visualize Data Structures</h3>
                            <p class="text-slate-500 max-w-md">Add values and run simulation to see how data structures behave internally.</p>
                            <!-- Ad Placement Empty State Pdraw -->
                            <div class="mt-8 hidden w-full mx-auto" data-ad-placement="pdraw_empty_state"></div>
                        </div>
                        ...
                        <div id="pdraw-results-wrapper" class="hidden h-full grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-0">
                            ...
                            <div class="lg:col-span-2 glass-panel rounded-xl p-6 flex flex-col max-h-[calc(100vh-6rem)] overflow-hidden order-last lg:order-none">
                                ...
                                <div id="pdraw-output" class="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
                                    <div class="flex flex-col items-center justify-center h-full text-center text-slate-500 opacity-50">
                            ...
                            <div class="lg:col-span-1 glass-panel rounded-xl p-4 flex flex-col max-h-[calc(100vh-6rem)] min-h-0">
                                <div class="border-b border-slate-700 pb-2 mb-2 flex justify-between items-center shrink-0">
            ...
            <script>
                document.getElementById('pdraw-simulate').addEventListener('click', function() {
                    document.getElementById('pdraw-empty-state').classList.add('hidden');
                    document.getElementById('pdraw-results-wrapper').classList.remove('hidden');
                    // Ensure the click also triggers the original simulation logic if it uses the same ID.
                    // If attached via addEventListener elsewhere, it will still work.
                });
            </script>
            ...
            <div id="tgdraw-dashboard" class="hidden transition-opacity duration-300 h-full flex flex-col">
                <div class="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 h-full overflow-y-auto lg:overflow-hidden">
                    <!-- Sidebar -->
                    <div class="lg:col-span-3 glass-panel rounded-xl p-4 flex flex-col space-y-4 h-auto lg:h-full">
                        <h3 class="text-sm font-bold text-amber-500 uppercase tracking-wider">TGDraw Lab</h3>
                        <p class="text-xs text-slate-500">Tree & Graph Visualizer</p>
                        <div class="p-4 bg-slate-800 rounded text-center text-slate-500 text-xs text-amber-500/50">
                            Phase 3 Configuration
                        </div>
                    </div>
                    <!-- Canvas -->
                    <div
                        class="lg:col-span-9 glass-panel rounded-xl p-4 flex flex-col relative overflow-hidden h-[500px] lg:h-full order-first lg:order-none">
                        ...
                        <div id="tgdraw-canvas"
                            class="flex-1 bg-slate-900/50 rounded flex items-center justify-center border border-slate-800">
